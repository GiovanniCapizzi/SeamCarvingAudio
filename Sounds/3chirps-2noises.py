...
# CHIRP 3 ------------------------------------------------
fi3 = 500.0                              # initial frequency
ff3 = 1200.0                              # final frequency
phi0 = 0                                
# linear frequency sweep
phi = phi0 + 2*np.pi*fi3*t + np.pi*(ff3 - fi3)/timeLength*t*t
s3 = Ar * np.cos(phi)
...
UF.wavwrite(zs, fs, "{0}s-chirps[{1}-{2};{3}-{4};{5}-{6}];noises[{7},{8},{9},{10}].wav".format(timeLength, fi1, ff1, fi2, ff2, fi3,ff3, noiseAmp, noiseWidth, noiseLocation, noiseLocation2))