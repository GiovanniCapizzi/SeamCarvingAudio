...
def popMode(mX:np.ndarray, pX:np.ndarray=None, N:int=4096, H:int=1024, fs:int=44100, debug:int=0)->tuple:
    ...
    freqs_bin, freqs_mag, freqs_phase = [],[],[]
    ...
    for i, j in short_seam:
        ...
        freqs_bin.append(f_bin * fs / N)    
        freqs_phase.append(f_phase)

        if f_mag < 1e-05:
            f_mag = 1e-05

        freqs_mag.append(f_mag)
    ...
    return 20*np.log10(freqs_mag), decay * H / fs , freqs_bin, freqs_phase     # convert and returns all values



if __name__ == '__main__':                          

    M = 8192                                             # window size, zero-phase-windowing if M<N
    N = 8192                                             # fft size, must be a power of two
    H = M//4                                             # hopsize, 1/4 of M
    auto = False                                         # compute the magnitude and phase of a generated sound (optional)

    inputFile = './cymbals1-mono.wav'
